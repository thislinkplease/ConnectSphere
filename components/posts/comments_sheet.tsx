...
export default function CommentsSheet(props: CommentsSheetProps) {
   ...
   useEffect(() => {
      ...
      (async () => {
         ...
         try {
            const all: Comment[] = await communityService.getAllPostComments(communityId, postId);

            const roots: Comment[] = all.filter((c: Comment) => c.parent_id === null);
            ...
            for (const root of roots) {
               newRepliesMap[root.id] = all.filter((c: Comment) => {
                  ...
                  const isChildOfReply = all.some((rc: Comment) => rc.parent_id === root.id && c.parent_id === rc.id);
            ...
            const usernames = Array.from(new Set(all.map((c: Comment) => c.author_username)));
   ...
   const handleLongPressComment = (comment: Comment) => {
      ...
      if (isOwnComment) {
         ...
         Alert.alert("Admin options", "Manage this comment", [
            {
               text: "Delete",
               style: "destructive",
               onPress: () => {
                  Alert.alert("Delete Comment", "Are you sure you want to delete this comment as an admin?", [
                     { text: "Cancel", style: "cancel" },
                     {
                        text: "Delete",
                        style: "destructive",
   ...
   const deleteComment = async (comment: Comment) => {
      try {
         await communityService.deletePostComment(communityId, postId, comment.id, me!.username);
   ...
   const sendComment = useCallback(async () => {
      ...
      try {
            ...
            try {
               ...
               if (replyTo.parent_id === null) {
                  setRootComments((prev) =>
                     prev.map((c: Comment) => (c.id === replyTo.id ? { ...c, content: input.trim() } : c))
                  );
               } else {
         ...
         console.warn("send comment error", e);
         if (e?.response?.status === 403) {
            Alert.alert(
               "Comment Blocked",
               e.response?.data?.message || "Your comment violates the community guidelines."
            );
            return;
         }
      } finally {
            ...
            >
               <Pressable onPress={() => handlePressUsername(reply.author_username)}>
                  <Text style={[styles.usernameText, { color: colors.text }]}>
                     {userData[reply.author_username]?.name || reply.author_username}
                  </Text>
               </Pressable>
            ...
            <View style={{ flex: 1 }}>
               ...
               >
                  <Pressable onPress={() => handlePressUsername(item.author_username)}>
                     <Text style={[styles.usernameText, { color: colors.text }]}>
                        {userData[item.author_username]?.name || item.author_username}
                     </Text>
                  </Pressable>
               ...
               {replies.length > 0 && (
                  <View style={{ marginTop: 8 }}>
                     {replies.slice(0, visibleReplies[item.id] ?? 0).map(renderReply)}

                     {visibleReplies[item.id] < replies.length && (
                        <Pressable onPress={() => loadMoreReplies(item.id)} style={{ paddingVertical: 6 }}>
   ...
   return (
      <Modal visible={visible} transparent animationType="none" onRequestClose={onClose}>
         ...
         {/* === BOTTOM SHEET === */}
         <Animated.View
            style={[styles.sheet, { transform: [{ translateY }], backgroundColor: colors.card }]}
         ...
         >
            ...
            <View
               style={[
                  styles.inputRow,
                  { marginBottom: keyboardHeight, backgroundColor: colors.card, borderTopColor: colors.border },
               ]}
            >
               {me?.avatar ? (
               ...
               <Pressable onPress={sendComment} disabled={submitting || !input.trim()}>
                  <Ionicons name="send" size={22} color={input.trim() ? colors.primary : colors.disabled} />
               </Pressable>
            </View>
         </Animated.View>
      </Modal>
