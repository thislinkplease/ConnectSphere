 ...
 */
export const formatTime = (date: string | Date | undefined | null, pattern: string = "HH:mm"): string => {
   if (!date) return "";
   const dateObj = typeof date === "string" ? parseISO(date) : date;
   if (!(dateObj instanceof Date) || isNaN(dateObj.getTime())) return "";
 ...
 */
export const formatDate = (date: string | Date | undefined | null, pattern: string = "MMM dd, yyyy"): string => {
   if (!date) return "";
   const dateObj = typeof date === "string" ? parseISO(date) : date;
   if (!(dateObj instanceof Date) || isNaN(dateObj.getTime())) return "";
   return format(dateObj, pattern);
...
export function toUTCString(localDate: Date) {
   const tzo = localDate.getTimezoneOffset() * 60000;
   return new Date(localDate.getTime() - tzo).toISOString();
}

export function fromUTCString(utc: string) {
   const d = new Date(utc);
   const tzo = d.getTimezoneOffset() * 60000;
   return new Date(d.getTime() + tzo);
}
